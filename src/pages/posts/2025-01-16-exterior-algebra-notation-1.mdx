...
\mathbf{u} \wedge \mathbf{v} &=  (u^1 \mathbf{e}_1 + u^2 \mathbf{e}_2) \otimes (v^1 \mathbf{e}_1 + v^2 \mathbf{e}_2) - (v^1 \mathbf{e}_1 + v^2 \mathbf{e}_2)\otimes (u^1 \mathbf{e}_1 + u^2 \mathbf{e}_2) \\
  &= (u^1 v^2 - v^1 u^2)(\mathbf{e}_1 \otimes \mathbf{e}_2) + (u^2 v^1 - v^2 u_1)(\mathbf{e}_2 \otimes \mathbf{e}_1)\\
  &= \sum_{i_1, i_2} \epsilon^{i_1, i_2} u^{i_1}v^{i_2}\mathbf{e}_{i_1 \otimes i_2} \\
  &= \sum_I \epsilon^I u^{i_1}v^{i_2}\mathbf{e}_{\otimes I} \\

\end{aligned}
$$

In the last line we are stuck using $i_1, i_2$ to refer to the explicit indices within the multi-index $I$.

**Permutation Basis**. The basis of $$\mathbf{e}_{\wedge I}$$, i.e. as (1) but with wedge products in the place of tensor products. The indices still range over all $$N^k$$ sequences of indices, but only those without any duplicates survive, for a total of $$\frac{N!}{(N-k)!}$$ components. Such an expression is manifestly antisymmetric as a tensor, although its coefficients are not necessarily antisymmetric in their indices; they will, however, be manifestly antisymmetric when the tensor results from a wedge product of vectors. I'll call this the "permutation basis", because all permutations of each possible *set* of indices are included in it.

There are two variants of this; the first is


$$
\begin{aligned}
\mathbf{u} \wedge \mathbf{v}  &= u^1 v^2 (\mathbf{e}_1 \wedge \mathbf{e}_2) + u^2 v^1 (\mathbf{e}_2 \wedge \mathbf{e}_1)\\
  &= \sum_{i_1, i_2} u^{i_1}v^{i_2}\mathbf{e}_{i_1 \wedge i_2} \\
  &= \sum_I u^{i_1}v^{i_2}\mathbf{e}_{\wedge I} \\
\end{aligned}
$$

while the second has antisymmetric coefficients therefore has to divide by $$\frac{1}{k!}$$ to avoid double-counting:

$$

\begin{aligned}
\mathbf{u} \wedge \mathbf{v}  &= \frac{1}{2}((u^1 v^2 -v^2 u^1) (\mathbf{e}_1 \wedge \mathbf{e}_2) + (u^2 v^1 -u^1 v^2)(\mathbf{e}_2 \wedge \mathbf{e}_1))\\
  
  &= \frac{1}{2} \sum_{i_1, i_2, j_1, j_2}  \epsilon^{i_1, i_2}_{j_1, j_2} u^{j_1}v^{j_2}\mathbf{e}_{i_1 \wedge i_2} \\
  &= \frac{1}{2} \sum_{I, J} \epsilon_J^I u^{j_1}v^{j_2}\mathbf{e}_{\wedge I} \\
\end{aligned}
$$

Obviously the former is much cleaner.
...
$$
\begin{aligned}
\mathbf{u} \wedge \mathbf{v}  &= (u^1 v^2 - u^2 v^1) (\mathbf{e}_1 \wedge \mathbf{e}_2)\\
&= (u^2 v^1 - u^1 v^2) (\mathbf{e}_2 \wedge \mathbf{e}_1) \\
&= \sum_{i_1 \wedge i_2, j_1, j_2}  \epsilon^{i_1, i_2}_{j_1, j_2} u^{j_1}v^{j_2}\mathbf{e}_{i_1 \wedge i_2} \\
&= \sum_{\wedge I, J} \epsilon_J^I u^{j_1}v^{j_2}\mathbf{e}_{\wedge I} \\
\end{aligned}
$$
