...
Note the use of $$\wedge K$$ indices and Einstein notation, which implies that the sum is in the combination basis. This would be the answer except that it's *no longer* in the combination basis: $$e_{\wedge K} \wedge e_{\wedge L}$$ are not a distinct basis elements. Instead we need sum over distinct basis elements $$e_{\wedge I}$$ and then, for each, include one set of indices $$K, L$$ which comprise it, with an appropriate sign. We get 
