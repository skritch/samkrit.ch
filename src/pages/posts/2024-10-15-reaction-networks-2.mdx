...
title: Reaction Networks, Part 2
...
We can test this guess by plugging $$\langle \bar{N} \rangle$$ into the Master equation and seeing what we get. We'll work in the g.f. representation where $$\psi = \sum_\bar{n} \psi_\bar{n} \bar{x}^\bar{n}$$, wnere expectations like $$\langle f \rangle = \sum_\bar{n} f(\bar{n}) \psi_\bar{n}$$ are calculated by setting $$\bar{x} = 1$$. Then we have:
...
The first discrepancy is the kind of thing that arise from the discreteness of $$\bar{n}$$ and would disappear in an $$N\to\infty$$ limit; we'll almost always have $$\bar{N} \gg \bar{k}$$, so the differences between $$\bar{n}^\bar{k}$$ and $$\bar{n}^{\bar{k}\downarrow}$$ ought will be minimal. 
...
This now has the same denominator as state $$\psi_{\bar{n}-\bar{k}}$$ will under the same ansatz. If additionally $$f(\bar{n})$$ is taken to be some specific tuple $$\bar{\lambda}$$ to the power $$\bar{n}$$, then we get even closer—we get a constant multiple of the exact state $$\psi_{\bar{n}-\bar{k}}$$:
...
So our full distribition is a product of independent Poissons with means: $$\bar{\lambda} = \langle \bar{N} \rangle$$; of course I knew this when I called them $$\lambda$$. In tuple notation we can write this whole combination as $$\bar{n} \sim \text{Poisson}(\bar{\lambda})$$. 
...
* Can we go the other way—does any solution to the Rate Equation $$\bar{X}(t)$$ always produce a Poisson state $$e^{\bar{X} \cdot \bar{x} - \bar{X}}$$ which is an equilibria of the Master Equation?
...
Well, there is is our condition: the term in parentheses must vanish. The obvious way for this to happen is for each $$\bar{k}_r' = \bar{k}_r$$, but that would not be very interesting; it would mean all of our reactions do nothing.
...
This condition is called "complex-balance". This is the "Anderson-Craciun-Kurtz" or "ACK" theorem, and are given in this Q.M.-derived notation in [this paper](https://arxiv.org/pdf/1305.4988) of Baez. (The proof in the paper seems needlessly complicated, though—or mine is in error!)
...
To say this another way—we have encountered three kind of Rate-Equation solutions:
...
Note also that the Rate Equation will be satisfied for any $$\bar{X}$$ wth $$\beta {(X_A)}^2 = \alpha X_B$$. At any such concentrations the rates of the two reactions are equal and the net change in concentration in zero. 
...
Our three $$c$$ coefficients, however, are complicated functions of $$n$$ and $$z$$, so it is very unlikely this would expression would be the discrete form of any reasonable continuous equation.

I wonder at this point:
* Can we say anything about the relationship between the unique equilbria of neighboring $$z$$ values?
...
* Furthermore, time-evolution will operately separately on components of distinct $$z$$, and any equilibria will be a linear combination of independent equilibria on each component.
...
Then the ACK theorem implies that the following Poisson state is an equilibrium solutiom to the Master equation:
