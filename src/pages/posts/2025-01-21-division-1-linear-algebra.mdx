...
title: "Division 1: Linear Algebra"
date: 2025-01-21
math: true
toc: true
...
1. $$x$$ is the number $$\frac{b}{a}$$.
2. ... unless $$a$$ is zero, in which case $$x = \frac{1}{0}$$ and there's no solution.
3. ... unless $$b$$ is also zero, in which case $$x = \frac{0}{0}$$ and any number is a solution.
...
(2) and (3) are the same special cases as in scalar division.
...
c. or as a function giving a solution given a choice of vector in the orthogonal subspace: $$\lambda \mapsto \mathbf{x}(\lambda)$$

Of these, (a) is most commonently seen, but in this post I'll prefer (c).

My preferred notation will be as follows: the full inverse be represented by $${(\mathbf{a} \cdot ~)}^{-1}$$, and depending on the context can have any of the above three interpretations as a set, a standard part + a member of a set, or a function to a single value.
A "fraction" or "division" notation $$\frac{b}{\mathbf{a}}$$ will always stand for the "standard" or "best" part of the inverse. We then used $$\mathbf{\lambda}_{\perp \mathbf{a}}$$ for the "orthogonal" or "complement" term:
...
The "division" notation is really the point here. This "generalized inverse" with a "division" for a "standard part", will be our 4th "prototype" case of division:

**Case 4**: Division gives a solution but only up to the addition of a term which multiplies to zero, for whatever definition of "multiplies" we're currently using.
...
$$
\begin{aligned}
\frac{df}{dx} = g \longrightarrow f &= {\left(\frac{d}{dx}\right)}^{-1} g \\
&= \int g~dx + C
\end{aligned}
$$
...
**Case 6**: Division is defined within a larger space of solution.
...
is more parameters than we need: for any pair of $$\mathbf{a}, \mathbf{b}$$, we can solve $$X\mathbf{a} = \mathbf{b}$$ by applying an appropriate combination of the identity $$I$$ and rotatoin $$R$$ within the plane spanned by the two vectors, while taking parameters on the rest of the space, which themselves could represent any combination of scalings and rotations we like. 
...
* when we wanted to put coordinates on the $$\mathbf{\lambda}$$ term in the generalized inverse $$\mathbf{x} = \frac{b}{\mathbf{a}} + E_{\perp a} \vec{\lambda}$$
...
For now, $$A$$ as the matrix with orthogonal basis vectors $$\mathbf{A}_i$$ as columns. Then the solution vector $$\mathbf{x}$$ will represent "the coordinates of $$\mathbf{b}$$ in $$A$$":
...
That is the simplest case: the matrix is perfectly invertible, so its "generalized inverse" $$A^{-1}$$ is exactly equal to its standard part $$\frac{1}{A}$$ and the therefore $$A\frac{b}{A}$$ is the identity. This is akin to case (1) from the scalar-over-scalar example.
...
Thus we have $$\frac{1}{A} = \frac{A^T}{A^T A}$$, which is (the simplest form of) the "Moore-Penrose pseudoinverse", normally witten $$A^+$$. I much prefer the division notation, and I find the symmetry between these the matrix and vector notations to be quite clarifying.
...
which can't be solved in general, unless the $${b}^i$$ happen also to also be zero.
...
It will be easier to writing these in block notation, so the above is equivalent to:
...
We can then "solve for" any single $$x^i$$ by wedging both sides of this expression with all of the other columns of $$A$$. The "[star index](https://samkrit.ch/2025/01/17/exterior-algebra-notation-2.html)" $$\mathbf{A}_{\star i}$$ represents the ordering of columns-other-than-$$i$$ such that $$\mathbf{A}_{\star i} = \mathbf{A}_{(\star i)_1} \wedge \mathbf{A}_{(\star i)_2} \wedge \ldots \wedge \mathbf{A}_{(\star i)_{N-1}}$$gives determinant times the $$N$$-volume: $$(\det A)\mathbf{e}_{\wedge 1..N}$$. This can also be seen as the "$$(N-1)$$th" wedge power of the matrix acting on the complement of the basis vector $$\mathbf{e}_i$$, written $$A^{\wedge (N-1)} (\star \mathbf{e}_i$$), where $$\star$$ is the "Hodge Star" operator which gives the $$(N-1)$$-dimensional volume orthogonal to a vector. 

All of the other terms $$\mathbf{A}_j \wedge \mathbf{A}_{\star i} = 0$$ for $$j\ne i$$ because the column $$\mathbf{A}_j$$ appears twice in wedge product. The only one that survives is the $$\mathbf{A}_i$$ term:
...
We therefore choose a value of $$x^1$$ such that its projection onto $$\perp A_{2..N}$$ gives $$\mathbf{b}_{\perp A_{2..N}}$$.
...
In total we have toured the following instances of division:
- $$ax = b$$ gave scalar-over-scalar division: $$b = x/a$$
...
4. The free parameter, which was required when (2) or (3) occured along some dimensions of the problem
