...
and this thing is simply the sum of the entropies $$H[p] + H[q]$$ iff both sets are normalized to 1. So this is the rule I was thinking of, but it only applies to distributions. Then the product of $$N$$ _independent_ copies of the same distribution would be $$H[z \times z \times \ldots] = NH[z^{N-1}]$$. But this corresponds to a g.f. $$(x_0 + y_0)(x_1 + y_1) \cdots (x_N + y_N)$$, rather than the g.f. $${(x+y)}^N$$, which has additional structure.  (TODO: check this I got confused)
