...
$$
\begin{align}
H[p(x)] &= -\sum _x\frac{e^{-\beta E}}{Z(\beta)} \ln \frac{e^{-\beta E}}{Z(\beta)}\\
     &= \left(\sum_x p(x) \ln Z(\beta)\right) + \left(\sum p(x) \beta E\right)\\
H[p(x)] &= \ln Z + \beta \langle E \rangle
\end{align}
$$
...
* $$H[p(x)] = S(U) = \ln \Omega(U)$$ admits interpretation as the Shannon entropy of *either*:
...
1. choosing one of $$Z$$ total states of $$A \times B_A$$.
2. choosing a state $$x$$ of systsem $$A$$ with probability $$p(x)$$, then for each $$x$$ choosing one of the $$e^{-\beta E(x)}$$ states of the reservoir $$B_A$$.
...
Here $$H[p]$$ is the Shannon entropy $$- \sum p \ln p$$.
