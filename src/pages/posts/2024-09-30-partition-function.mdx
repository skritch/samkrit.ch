...
Systems which are not independent will have some conditional structure to their multiplicities under the microcanonical hypothesis, but will still be uniformly distributed within that slice of the space:

$$
p(x_A, x_B) = p(x_A)p(x_B \mid x_A) \sim \mathrm{Uniform}(\Omega_A(x_A) \cdot (\Omega_B \mid x_A))
$$
...
Let's now look at the standard "heat reservoir" derivation of the partition function. We imagine a system $$A$$ which can exchange energy freely with a much larger reservoir $$B$$. We label the states of each by their energies $$E_A, E_B$$, and each has a multiplicity function which depends only on its energy: $$\Omega_A(E_A), \Omega_B(E_B)$$. I will be departing a bit from the standard textbook argument by making liberal use of "generating functions" ("g.f."s from now on). So we will represent the states of $$A$$ and $$B$$ separately as two generating functions:
...
where $$y$$ and $$z$$ are placeholder variables which track the "energy cost" of each state, which is a standard g.f. method. These should be thought of as "transformations" of the underlying series $$\Omega_A$$ and $$\Omega_B$$.
...
* We can also plug in other values for $$y$$ to assign a "cost" to each unit of $$E$$. This is best illustrated with the binomial series $${(x+y)}^N = \sum {N \choose k} x^k y^{N-k}$$ as seen in the following examples:
  ...
  * Plugging in $$x = y$$ marks the two variables as indistinguishable, reducing the series to $$(2x)^N$$.
...
$$
\begin{align}
g_{A \times B}(y, z) &= g_A(y)\times g_B(z)  \\
  &= \left(\sum_{E_A} \Omega_A(E_A) y^{E_A} \right)\left(\sum_{E_B} \Omega_B(E_B) z^{E_B} \right)
\end{align}
$$
...
Now that we have our joint g.f., we'll add the assumption that the reservoir $$B$$ is so much larger than system $$A$$, such that, its state will not change appreciably as energy is exchanged (though the multiplicity $$\Omega_B$$ _will_ vary). Thus the *rate* of the variation of $$\Omega_B$$ will be approximately constant, and we can replace the function $$\Omega_B$$ with a linear approximation. 

We'll actually linearize $$S_B$$ rather than $$\Omega_B$$. I'll only try to justify this with a hand-wave-y argument for now: entropies compose additively, as in $$S_{A\times B} = S_A + S_B$$, so they represent "extensive" physical quantities and are therefore "the sorts of thing we take derivatives of". Multiplicities compose multiplicatively, and are rightly handled with logarithmic derivatives. More on this later.
...
The rectangles widths grow according to $$\Omega_A$$ while their heights shrink according to to $$e^{-\beta E_A}$$. 

Evidently, the partition function $$Z$$ is the combined area of the all of the rectangles, being the sum of a products of factors $$\Omega_A(E_A)$$ and $$e^{-\beta E_A}$$ [^1]
...
The joint multiplicities $$\Omega(E) = \Omega_A(E) e^{-\beta E}$$ are _areas_, which grow and then shrink again:

![Boltzmann areas](../../images/partition/boltzmann-areas-graph.png)

The "third" system $$B_{\not{A}} = C$$ can be visualized as a third dimension which does *not* vary with $$E_A$$:

![boltzmann visulialized with constant 3rd dimension](../../images/partition/3d-boltzmann-cupola.png)

This third dimension does not participate in any of our 
...
Strictly speaking, the above equation is not how the Legendre transform is usually written. The standard form would be $$\frac{F(T)}{T} = \min_{E} \frac{1}{T}E - S(E)$$, which may be read as a Legendre transform of $$S(E)$$ w.r.t. a slope named $$\frac{1}{T}$$. This definition can be written more naturally in terms of $$\beta = \frac{1}{kT}$$ with a "dimensionless free energy" $$\mathcal{F} = \beta F$$ and "dimensionless entropy" $$\mathcal{S} = S/k$$, giving: 

$$
\mathcal{F} = \min_E \beta E - \mathcal{S}(E)
$$

Alternatively, you could start with the $$S(E)$$ inverted as $$E(S)$$ and Legendre transform $$E(S) \to F(T)$$ by way of the the parameter $$T = \frac{d E}{d S}$$. These only amount to different ways of rearranging the underlying differential $$dF = dE - TdS + SdT$$. 
...
\\
&= M_{\text{uniform}}(0 + (-\beta))
\end{align}
$$

So $$\frac{Z(\beta)}{\Omega} = M_{\text{uniform}}(-\beta)$$ is the m.g.f. of a uniform distribution. This uniform distribution is not especially useful: it corresponds to $$\beta = 0$$, i.e. infinite temperature. Instead we can think of $Z(\beta)$ as an expansion around some $\beta^*$

$$
\begin{align}
Z(\beta) &= Z(\beta^* + (\beta - \beta^*)) \\
&= Z(\beta^* + d\beta) \\
&= Z(\beta^*) + Z'\vert_{\beta^*} \frac{d\beta}{1!} + Z''\vert_{\beta^*} \frac{{(d\beta)}^2}{2!} + \cdots
\\
&= \left(\sum \Omega(E)e^{-\beta^* E}\right) \\
&\quad\quad {+}\left(\sum (-E)\Omega(E)e^{-\beta^* E} \right)\frac{d\beta}{1!} \\
&\quad\quad {+} \left(\sum (+E^2)\Omega(E)e^{-\beta^* E} \right)\frac{ {(d\beta)}^2}{2!} + \cdots
\\
&= Z(\beta^*)\left(1 - {\langle E\rangle}_\text{Boltzmann}\frac{d\beta}{1!} + {\langle E^2 \rangle}_\text{Boltzmann}\frac{ {(d\beta)}^2}{2!} + \cdots\right) 
\end{align}
$$
...
\begin{align}
\frac{Z(\beta)}{Z(\beta^*)}
&= 1 - {\langle E\rangle}_\text{Boltzmann}\frac{d\beta}{1!}\\
 &\quad\quad {+}~ {\langle E^2 \rangle}_\text{Boltzmann}\frac{ {(d\beta)}^2}{2!}
...
Finally we have the series $$\ln Z$$. The series expansion of  $$\ln Z$$ appears to generate central moments, but this turns out not to hold after the third moment. This turns out to be a general property of the logarithm $$\ln M_X$$ of any mgf, called the "[cumulant generating function](https://en.wikipedia.org/wiki/Cumulant)". It is:
