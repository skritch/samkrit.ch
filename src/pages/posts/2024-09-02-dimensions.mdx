...
So whatever $$\mathbf{a}_\perp$$ means, it ought to able to play the role of "projection" and "rejection" equally well.
...
Is the polygon anything? I'm not sure. There's no way to draw the "vector itself" with any kind of arrow; we can only depict its components as a kind of polygon. And the diagram is defined only up to the signs of the components and up to the choice "projection" on any multi-dimensions (the $$3,4$$ dimension here.) In another basis, the sum of squared components would be unchanged, and of course there is any basis with a basis vector parallel to $$\mathbf{a}$$ itself will have only a single component and no "polygon" associated with it at all. But I do wonder if anything can be said about this polygon that *is* invariant under basis changes and the arbitrary orientations of the axes in  the diagram.

This kind of diagram might be more sensible for depicting probabilities, which can't be negative anyway. 
...
The first two eigenvalues are the biggest, so we could approximate this matrix by only its first two "principle components", i.e. by zeroing all but the first two eigenvectors, defining a new matrix $$A'$$:
...
This is something like a "principle component analysis". The action of $$A'$$ on a given input vector $$\mathbf{x}$$ will deviate from that of the original $$A$$ in some way that depends on $$\mathbf{x}$$'s components along the zeroed dimensions:
...
where the last expressoin is to be understood as a standard inner product.

We can equivalently think of this PCA-like-reduction as a reduction of the _vector space_ to three-dimensions, where the first two dimensions are chosen as principle eigendimensions. Under this reduction $$A$$ transforms into a reduced $$\tilde{A}$$:
