...
title: "Drawing Dimensions 2: Data Engineering"
date: 2025-08-30
math: true
toc: true
...
Here I'll try to apply the visualization methods from my <a href="/posts/2024-09-02-dimensions/">Drawing Dimensions</a> post to the domain of data engineering.
...
Obviously the former must be precede the latter; the difference is the latency:
...
Even an Excel spreadsheet counts, even if only be updated at 10 a.m. when someone or another gets into the office.
...
If no accumulating state is needed to calculate the next value from the previous one, the individual batches can be treated as distinct datasets—"partitions" of the full data. Ideally data is partitioned on its real, i.e. logical, time; this being the time dimension you most often want to query by. But realtime data can usually have arbitrarily-late arrival times (due to upstream errors and the like), and one must either accept incomplete data, or partition and process  by arrival time. If you want a clean dataset that's partitioned on logical time you have to merge the partitions later and allow that a newly-arriving partition can carry updates to many logical-time partitions at once. Databases can handle this; cloud storage not so much, not without specialized tools.
...
Here find another class of dimensions, representing another complicatoin: versioning. 
...
Furthermore the *outputs* of our modern AI models are stochastic. Anything downstream of an AI model have depend on a dimension representing the output it actually consumed, though this is likely redundant with the "run id" of the AI task and perhaps also with the exact filename.
