...
Its argument is a probability distribution, such $H[p_i]$, $H[p(x)]$, $H[\mathcal{N}(\mu, \sigma^2)]$, or a random variable $H[X]$ with its distribution given elsewhere. The square brackets $H[\:\cdot\:]$ should be read as indicating that it is a function of an entire distribution rather than of a single numeric value. 
...
the entropy goes up by $1$, in $\log_2$ bits.
...
For each block of weight $k_i$ and fraction $p_i = \frac{k_i}{K}$ of the whole, dividing it into two half-cells increases its information $I(p_i)$ by $1$ bit:
...
$$
\begin{align}
H\left[\frac{p_1}{2}, \frac{p_2}{2}, \ldots \right] &= 2 \times \frac{p_1}{2} \log{\frac{1}{p_1/2}} + 2 \times \frac{p_2}{2} \log{\frac{1}{p_2/2}} + \ldots \\
  &= p_1(\log\frac{1}{p_i} + 1) + p_2(\log\frac{1}{p_i} + 1) + \ldots\\
  &= H[p_1, p_2, \ldots] + (p_1 + p_2 + \ldots) \times 1 \\
  &= H[p_1, p_2, \ldots] + 1
\end{align}
$$

In the "addressing" or "labeling" interpretation, we think of this as saying: the distribution $\frac{p_1}{2}, \frac{p_2}{2}, \ldots$ can be addressed by first specifying an element with of $p_1, p_2, \ldots$ and then providing a single additional bit: 0 or 1, or "left" or "right", to pick between elements of the new subdivision.
...
We can go in the other direction to calculate the entropy of a coarse-graining or aggregating operation. For any distribution $p_i$ (ignoring zeros) there will be some common denominator with which we can write probability as a fraction: $\frac{k_i}{K}$. For example if our probabilities are $0.9, 0.099, 0.001$ we can write these as $\frac{900}{1000}, \frac{99}{1000}, \frac{1}{1000}$. Then we can imagine constructing this distribution by first subdividing into $K$ blocks, then merging blocks to wind up with our final distribution. Merging $k$ blocks will change the entropy from

$$
\begin{align}
H\left[ \frac{k}{K}, \ldots \right] &= \frac{k}{K} \log{ \frac{1}{k/K} } + \ldots\\
  &= \frac{k}{K} (\log{K} + \log{\frac{1}{k}}) + \ldots \\
  &= \bigg(\underbrace{\frac{1}{K} + \frac{1}{K} + \ldots}_{k \text{ times}}\bigg) \log{\frac{1}{1/K}} - \frac{k}{K} \log k + \ldots \\
  &= H\big[ \underbrace{\frac{1}{K}, \frac{1}{K}, \ldots}_{k \text{ times}}\big] - \frac{k}{K} \log k
\end{align}
$$

So grouping $k$ cells into one of size $\frac{k}{K}$ decreases the entropy by a fraction $\frac{k}{K}$ of $\log{k}$, here by $\frac{4}{12}$ of $\log{4}$ which is $0.66\bar{6}$:

<EntropyBar total={12} weights={[]} client:visible />
<EntropyBar total={12} weights={[4]} client:visible />

And grouping all $K$ cells into one of course brings the entropy down to zero:
<EntropyBar total={12} weights={[]} client:visible />
<EntropyBar total={12} weights={[12]} client:visible />


Note that the term subtracting in the above is exactly the (fraction occupied by the merged block) $\times$ (the entropy of a uniform distribution over the merged block). We can create any distribution of blocks by merging a uniform into groups $k_i$:

$$
\begin{align}
H\left[ \frac{k_i}{K}, \ldots \right] &= H\bigg[ \underbrace{\frac{1}{K}, \frac{1}{K}}_{k \text{ times}}, \ldots\bigg] - \sum_i \frac{k_i}{K} \log k_i \\
 &= H[\text{uniform}(K)] - \sum_i \frac{k_i}{K} H[\text{uniform}(k_i)]
\end{align}
$$
...
$$
H\left[ \frac{k_i}{K} \right] +  \sum_i \frac{k_i}{K} H[\text{uniform}(k_i)] = H[\text{uniform}(K)]
$$

in which we can read two the entropy of two equivalent algorithms:
1. either we first select a block from an uneven distribution $\{ \frac{k_i}{K} \}$, then select uniformly from within that block according to a $\text{uniform}(k_i)$
2. or, we select one element from a $\text{uniform}(K)$ distribution

Clearly we should be able to count or address a set of $K$ elements either way—these two selection processes are the same; they must contain or require the same amount of information, hence, their entropies must also be the same.[^derivingH]
...
... but the three-way partitions quickly overtake the two-way ones, and of course there are a *lot* of these. The total number of partitions is given by the [Bell numbers](https://oeis.org/A000110); the 16th Bell number is around 10 billion. But the number of distinct integer partitions (e.g. $\{14, 1, 1\}$) is far smaller, and all of these have the same entropy—so the problem turns out to be extremely tractable. Plus it's 2025, so I can ask an AI to do it in two minutes.[^chatgpt] Here's a visualization:
...
There's a clear shape, peaking (the AI tells me) near $\log(16) = 2.77$. Neat. (Note the log scale: at the peak value there appear to be about 800 million partitions!)
...
- I don't think a "uniform measure" makes sense on the space of real-valued probability distributions, so the second visualization just given is probably not very meaningful. It might actually make sense to use the entropy itself as the measure on that space, as $H$ is the asymptotic measure a distribution arising from samples from a uniform, by the third argument from the previous post in thisseries. But I won't try to take that on now.


# 4. Entropy vs. Variance

Here's one visualization of some entropies. Now—unlike all the earlier examples—we will associated the indices $i$ with their numeric value, i.e. treating this as a random variable (henceforth "r.v.") $X \sim p_i$ with $X(i) = i$. This will let us compute a variance for the distribution.
...
Here now are a family of $\text{Beta}(\alpha, \beta)$  distributions, which might represent the posterior distribution of the probability $p$ that an unfair coin comes up heads, which can be seen to converge to the true value I used in the simulation of $p=0.25$.
...
\text{Pr}(p \mid N) &\propto \text{Pr}(N \mid p) \times \text{Pr}(p) \\
  &\propto (\text{Beta}(N+1, 1) + \text{Beta}(1, N+1))\times \text{Beta}(1, 1) \\
